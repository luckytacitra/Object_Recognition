...
# =====================================================================
# PERBAIKAN:
# 1. Suara OCR: setiap teks berbeda, suara keluar (fix dedup)
# 2. Suara rintangan: cooldown + posisi filter
# 3. YOLO M1: objek umum, M2: lubang/tangga, M3: rambu (sesuai fungsinya)
# 4. Bahaya hanya jika objek di depan (pos_x 0.3-0.7)
# 5. Suara sampai akhir video (loop berjalan terus)
# =====================================================================
...
# ============================================================
# DETEKSI - MODEL SESUAI FUNGSI
# ============================================================
# PERBAIKAN: M1=YOLO umum, M2=lubang/tangga, M3=rambu
...
def process_frame_detection(frame, model, conf=0.4, model_type='general'):
    if model is None: return frame, []
    try:
        # PERBAIKAN: M2 conf lebih rendah untuk lubang/tangga
        ...
        if results and len(results) > 0:
            ...
            if result.boxes is not None and len(result.boxes) > 0:
                ...
                for box, conf_score, cls_idx in zip(boxes, confidences, classes):
                    ...
                    is_rambu_obj = is_rambu(cls_name)

                    area = (x2 - x1) * (y2 - y1)
                    area_ratio = area / (fw * fh)

                    # PERBAIKAN: filter area sesuai model
                    if model_type == 'obstacle':
                        if area_ratio < 0.003: continue
                        if conf_score < 0.10: continue
                    else:
                        if area_ratio < 0.015: continue

                    pos_x = ((x1 + x2) / 2) / fw

                    # PERBAIKAN: Bahaya hanya jika di DEPAN (0.3-0.7)
                    is_depan = 0.3 <= pos_x <= 0.7

                    # PERBAIKAN: Risk level
                    if is_obstacle and is_depan:
                        if area_ratio > 0.05:
                            risk_level = 'BAHAYA'
                            ...
                            risk_level = 'AMAN'
                    elif is_vehicle and is_depan:
                        if area_ratio > 0.10 and conf_score > 0.40:
                            risk_level = 'BAHAYA'
                        ...
                        # ORANG TIDAK PERNAH BAHAYA
                        if conf_score > 0.40 and area_ratio > 0.08 and is_depan:
                            risk_level = 'WASPADA'
                        else:
                            risk_level = 'AMAN'
                    elif is_rambu_obj:
...
def process_frame_detection_multi(frame, m1, m2, m3, conf=0.4):
    ...
    # PERBAIKAN: M2 = obstacle (lubang/tangga) conf 0.15
    if m2:
        out, d = process_frame_detection(out, m2, conf, model_type='obstacle')
        all_d.extend(d)
    
    # M1 = general
    if m1:
        out, d = process_frame_detection(out, m1, conf, model_type='general')
        all_d.extend(d)
    
    # M3 = rambu
    if m3:
        out, d = process_frame_detection(out, m3, conf, model_type='general')
        all_d.extend(d)
...
def handle_alerts(dets, now_time, enable_audio, warn_ph, status_ph, audio_ph, cooldown=5):
    ...
    rambu = [d for d in dets if d['risk_level'] == 'INFO' and is_rambu(d['class'])]
...
def handle_ocr(frame, ocr_eng, min_conf, en_tts, ocr_ph, aocr_ph):
    text = perform_ocr_on_frame(frame, ocr_eng, min_conf)
    if text and text != "Tidak ada teks terdeteksi" and len(text) > 2:
        # PERBAIKAN: threshold 0.4 agar lebih sensitif bedakan teks
        if not texts_are_similar(text, st.session_state.last_ocr_text, threshold=0.4):
...
with c2: st.markdown('<div class="header-text"><h1>Asisten Navigasi Tunanetra</h1><p>Deteksi Objek & Teks v5.2</p></div>', unsafe_allow_html=True)
st.divider()
...
with st.sidebar:
    ...
    conf_threshold = st.slider("Confidence M1", 0.1, 0.9, 0.35, 0.05, help="M2 (lubang) otomatis conf=0.15")
...
with tab1:
    ...
    if mode == "📹 Webcam":
        ...
        with col_btn1: run_cam = st.toggle("🔴 AKTIFKAN KAMERA")
        ...
        if run_cam:
            ...
            while run_cam:
                ...
                danger_list, rambu_list = handle_alerts(dets, now_t, enable_audio, warn_ph, status_ph, audio_alert_ph, cooldown=5)
                
                if st.session_state.ocr_triggered_cam and ocr_eng is not None:
                    if cnt % ocr_scan_interval == 0:
                        handle_ocr(orig, ocr_eng, ocr_min_conf, enable_tts, ocr_ph, audio_ocr_ph)
        ...
        if uploaded and btn_start:
            ...
            try:
                ...
                while True:
                    ...
                    now_time = time.time()
                    danger_list, rambu_list = handle_alerts(dets, now_time, enable_audio, warn_ph, status_ph, audio_alert_ph, cooldown=5)
                    ...
                    if st.session_state.ocr_triggered_vid and ocr is not None:
                        ...
                        if st.session_state.ocr_frame_count % ocr_scan_interval == 0:
                            handle_ocr(orig, ocr, ocr_min_conf, enable_tts, ocr_ph, audio_ocr_ph)
...
<div style="text-align:center; color:#999; font-size:0.8rem; padding:1rem 0;">
    <strong>Asisten Navigasi Tunanetra v5.2</strong> • YOLOv11 • EasyOCR • gTTS
</div>
""", unsafe_allow_html=True)
