# =====================================================================
# ASISTEN NAVIGASI TUNANETRA - STREAMLIT v6.2 (FULL)
# =====================================================================
# GABUNGAN LENGKAP: v6.0 (Colab logic) + v5.2 (suara stabil) + webcam
# =====================================================================
...
.stButton > button { border-radius: 10px; border: none; font-weight: 600; padding: 0.8rem 1.4rem; transition: all 0.2s; text-transform: uppercase; letter-spacing: 0.5px; }
    ...
    try:
        ...
        if results and results[0].boxes is not None:
            ...
            for box,cs,ci in zip(r.boxes.xyxy.cpu().numpy(),
                ...
                cn=r.names[ci]
                cn_matched = None
                for kb in KELAS_BEST_DIPAKAI:
                    if kb.lower() == cn.lower():
                        cn_matched = kb; break
                if cn_matched is None: continue
                cn = cn_matched
                x1,y1,x2,y2=map(int,box)
                fh,fw=frame.shape[:2]
                ar=(x2-x1)*(y2-y1)/(fw*fh)
...
def do_alerts(dets, en_audio, cooldown, w_ph, s_ph, ad_ph, ar_ph):
    ...
    if bahaya and en_audio:
        ...
        if (now - st.session_state.last_alert_time[k]) >= cooldown:
            ...
            w_ph.markdown(f'<div class="alert-danger">🚨 {nama} masih ada!</div>',unsafe_allow_html=True)
...
with c2: st.markdown('<div class="header-text"><h1>Asisten Navigasi Tunanetra</h1><p>Deteksi Objek + Baca Teks — v6.2 (Full)</p></div>',unsafe_allow_html=True)
st.divider()
...
with st.sidebar:
    ...
    if st.button("📥 Load YOLO M1", use_container_width=True):
        with st.spinner("Loading..."): 
            st.session_state.model1 = load_yolo()
            add_log("YOLO loaded")
            st.success("✅ YOLO M1 Dimuat!")
    ...
    with st.expander("🏔️ M2: best.pt (Tangga/Lubang)"):
        ...
        if up2 and st.button("Load M2", use_container_width=True):
            with st.spinner("Memuat M2..."):
                ...
                add_log(f"M2 loaded: {len(names)} classes")
                st.success(f"✅ M2 Dimuat! {len(names)} kelas")
    
    with st.expander("🚦 M3: best_rambu.pt (Rambu Indonesia)"):
        up3 = st.file_uploader("Upload best_rambu.pt", type=['pt'], key='m3up')
        if up3 and st.button("Load M3", use_container_width=True):
    ...
    if st.session_state.m2_classes:
        st.caption(f"M2 kelas: {', '.join(sorted(st.session_state.m2_classes))}")
    ...
    frame_skip = st.slider("Frame Skip", 1, 10, 2, help="Default 2. Naikkan kalau lambat.")
    ...
    if mode == "📹 Webcam":
        ...
        if cam:
            if not st.session_state.model1 and not st.session_state.model2:
                ...
                if show_logs:
                    lp.markdown('<br>'.join([f'[{t}] {m}' for t, m in st.session_state.log[:10]]), unsafe_allow_html=True)
        ...
        if uploaded:
            ...
            if st.session_state.last_uploaded_name != cn:
                ...
                st.session_state.last_uploaded_name = cn
        ...
        if uploaded and btn_start:
            if not st.session_state.model1 and not st.session_state.model2:
                st.error("⚠️ Load YOLO dulu!"); st.stop()
            ...
            try:
                ...
                t0 = time.time()
                prog = st.progress(0)
                OCR_INTERVAL = int(fpsv * 3)  # scan OCR tiap 3 detik
                ...
                while True:
                    ...
                    # Resize agar deteksi & OCR akurat
                    ...
                    # ALERT — SUARA KELUAR SETIAP ADA OBJEK
                    dl, rl = do_alerts(dets, enable_audio, alert_cooldown, wp, sp, adp, arp)
                    
                    # OCR — scan tiap 3 detik
                    if st.session_state.ocr_triggered and ocr_eng:
                        st.session_state.ocr_frame_count += 1
                        frames_per_3s = max(int(fpsv * 3 / frame_skip), 1)
                        if st.session_state.ocr_frame_count % frames_per_3s == 0:
                    ...
                    mf.metric("FPS", f"{cnt/elapsed:.1f}" if elapsed > 0 else "0")
                    
                    if show_logs:
                        lp.markdown('<br>'.join([f'[{t}] {m}' for t, m in st.session_state.log[:10]]), unsafe_allow_html=True)
                    
                    # Delay agar video berjalan normal
                    delay = (frame_skip / fpsv)
...
with tab2:
    ...
    if mode2 == "📷 Kamera":
        cm = st.camera_input("📸 Foto untuk baca teks")
        if cm:
            if not st.session_state.ocr_engine:
...
<div style="text-align:center; color:#999; font-size:0.8rem; padding:1rem 0;">
    <strong>Asisten Navigasi Tunanetra v6.2</strong> • YOLOv11 • EasyOCR • gTTS
</div>
""", unsafe_allow_html=True)
