...
def process_frame_detection_multi(frame, m1, m2, m3, conf=0.4):
    ...
    all_d = []
    if m1:
        out, d = process_frame_detection(out, m1, conf, source='m1')
        all_d.extend(d)
    if m2:
        out, d = process_frame_detection(out, m2, conf, source='m2')
        all_d.extend(d)
    if m3:
        out, d = process_frame_detection(out, m3, conf, source='m3')
        all_d.extend(d)
    return out, all_d
